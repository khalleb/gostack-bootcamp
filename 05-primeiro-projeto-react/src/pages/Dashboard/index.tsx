...
const Dashboard: React.FC = () => {
  ...
  const [repositories, setRepositories] = useState<Repository[]>(() => {
    const storagedRepositories = localStorage.getItem('@GitHubExporer:repositories');
  ...
  async function handleRepositories(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!newRepo) {
      setInputError('Digite o autor/nome do repoistório');
      return;
    }
  ...
  return (
    <>
      ...
      <Form hasError={!!inputError} onSubmit={handleRepositories}>
        <input
          value={newRepo}
